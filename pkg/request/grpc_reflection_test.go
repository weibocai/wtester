...
func TestGrpcClientPool(t *testing.T) {
	...
	if _, err := RegisterGrpcReflectionClient(url, serviceName, methodName); err != nil {
	...
	for i := 0; i <= 10; i++ {
		group.Go(func() {
			for j := 0; j <= 10; j++ {
				if res, err := GrpcReflectionRequest(url, serviceName, methodName, requestData); err != nil {
